...
class EconomyCommandExecution(private val main: Main) {
	...
	fun addFriend(player: Player, args: Array<Any>) {
		...
		if (target.uniqueId == player.uniqueId) {
			...
			ChatComponentAPI().appendText("§7[Click here to try someone else]").withClickEvent("/economy friend add ", ClickEvent.Action.SUGGEST_COMMAND).sendToPlayer(player)
			return
		}
		...
		ChatComponentAPI().appendText("§7Accept friend request from §6" + player.name + "§7?").appendTextComponent(ChatComponentAPI().appendText("§a[YES]").withClickEvent("/economy friend accept " + player.name, ClickEvent.Action.RUN_COMMAND).showTextOnHover("§7By clicking this you are going to §aaccept §7this friend request")).appendTextComponent(ChatComponentAPI().appendText("§c[NO]").withClickEvent("/economy friend deny " + player.name, ClickEvent.Action.RUN_COMMAND).showTextOnHover("§7By clicking this you are going to §cdeny §7this friend request")).sendToPlayer(target)
	}
	...
	fun removeFriend(player: Player, args: Array<Any>) {
		...
		if (target.uniqueId == player.uniqueId) {
			player.sendMessage("§cYou cannot use this command on yourself!")
			ChatComponentAPI().appendText("§7[Click here to try someone else]").withClickEvent("/economy friend remove ", ClickEvent.Action.SUGGEST_COMMAND).sendToPlayer(player)
			return
		}
		if (!main.friend.isFriend(player.uniqueId, target.uniqueId)) {
	...
	fun acceptFriend(player: Player, args: Array<Any>) {
		...
		if (player.uniqueId == target.uniqueId) {
			player.sendMessage("§cYou cannot use this command on yourself!")
			ChatComponentAPI().appendText("§7[Click here to try someone else!]").withClickEvent("/economy friend accept ", ClickEvent.Action.SUGGEST_COMMAND).sendToPlayer(player)
			return
		}
	...
	fun denyFriend(player: Player, args: Array<Any>) {
		...
		if (player.uniqueId == target.uniqueId) {
			player.sendMessage("§cYou cannot use this command on yourself!")
			ChatComponentAPI().appendText("§7[Click here to try someone else!]").withClickEvent("/economy friend deny ", ClickEvent.Action.SUGGEST_COMMAND).sendToPlayer(player)
			return
		}
		if (!main.friendRequest.isFriendRequest(target.uniqueId, player.uniqueId)) {
			player.sendMessage("§7You don't have an incoming friend request from §6" + target.name + "§7!")
