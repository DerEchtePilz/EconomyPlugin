...
class Config(private val main: Main) {
	...
	fun loadConfig() {
		if (isLoaded) {
			main.logger.info(
				Ansi().fgRed().a("The config is already loaded! Please do not call the loadConfig() method twice!")
					.toString()
			)
			return
		}
		...
		defaultConfigValues =
			gson.fromJson(getValuesFromDefaultConfig(), HashMap::class.java) as MutableMap<String, String>
