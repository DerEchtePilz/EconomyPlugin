...
enum class Permission(private val permission: String, private val id: Int, private val id200: Int) {
	...
	companion object {
		...
		fun hasPermission(player: Player, permission: Permission): Boolean {
			if (player.persistentDataContainer.has(NamespacedKeys.PERMISSION, PersistentDataType.INTEGER_ARRAY)) {
				val permissions: IntArray? = player.persistentDataContainer.get(NamespacedKeys.PERMISSION, PersistentDataType.INTEGER_ARRAY)
		...
		fun addPermission(player: Player, permission: Permission) {
			...
			val playerPermissions = if (player.persistentDataContainer.has(NamespacedKeys.PERMISSION, PersistentDataType.INTEGER_ARRAY)) {
			...
			player.persistentDataContainer.set(
				NamespacedKeys.PERMISSION, PersistentDataType.INTEGER_ARRAY, updatedPermissions
			)
		}
