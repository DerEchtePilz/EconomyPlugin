...
class TestsCommand(main: Main) {
    ...
    fun register() {
        ...
        CommandAPICommand("argumentrequirementtest")
            .withArguments(LiteralArgument.of("test4"))
            .withArguments(StringArgument("requirement").withRequirement { sender: CommandSender -> sender.isOp })
            .withArguments(StringArgument("test5"))
        ...
        CommandTree("reqs")
            .then(LiteralArgument.of("test1")
                .withRequirement { sender: CommandSender -> sender.isOp }
                .then(LiteralArgument.of("test1arg")
                    .executesPlayer(PlayerCommandExecutor { player, args ->
                        player.sendMessage("Requirement works!")
                    })
                )
            )
            .then(LiteralArgument.of("test2")
                .then(LiteralArgument.of("test2arg")
                    .executesPlayer(PlayerCommandExecutor { player, args ->
                        player.sendMessage("Requirement works!")
                    })
                )
